...
public class TaskServiceImpl implements TaskService {
    ...
    public boolean createTask(CreateOrUpdateTaskDTO taskDTO) {
        ...
        if (currentUser == null) {
            throw new ParameterException(10030);  // 用户未登录
        }
        taskDO.setCreator(currentUser.getId());
        ...
        for (Integer machine : machines) {
            ...
            if (machineDO.getClientId() == null || machineDO.getClientId().trim().isEmpty()) {
                log.warn("机器{}没有有效的SocketIO连接，clientId为空", machineDO.getName());
                throw new ParameterException(12301);
            }
        ...
        if (taskMapper.insert(taskDO) > 0) {
            // 将备选压力机加入room,发送启动命令
            List<String> clientIds = machines.stream()
                .map(mid -> machineMapper.selectById(mid).getClientId())
            ...
            for (String clientId : clientIds) {
                try {
                    ...
                    if (client != null) {
                        ...
                        log.warn("SocketIO客户端未找到，clientId: {}", clientId);
                    }
                } catch (IllegalArgumentException e) {
                    log.error("无效的clientId格式: {}, 跳过该机器", clientId);
                }
            }
